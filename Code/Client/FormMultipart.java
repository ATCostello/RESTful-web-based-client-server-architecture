...
public class FormMultipart extends FormParser {
	...
	// TODO: this is a bit too simple for multipart headers; we need to do an
	// explode that preserves strings using quote marks...
	...
	FormData getFormData(String contentType, byte payload[]) {
		...
		// extract the boundary delimiter, which is a sub-field of the contentType
		// header
		...
		// - after this will be a "\r\n" (meaning a field) or a "--" (meaning the end of
		// the fields)
		// - for a field, there are zero or more header lines, each terminated by \r\n,
		// then a blank line with \r\n
		// - we then have the actual data of this field, up to the next boundary field,
		// where we repeat the above
		...
		while (true) {
			...
			// - in the former case, we're about to read a new section, else we're at the
			// end
			...
			if (Arrays.equals(Arrays.copyOfRange(payload, next, next + 2), "\r\n".getBytes())) {
				...
				// - we do this by checking for a field called "filename" or a content-type
				// header
				...
				if (headerExists(subh, "filename") || headerExists(headers, "content-type")) {
					...
					System.out.println(nff.name);
					System.out.println(nff.content);
					System.out.println(nff.filename);
				...
				// this is really a syntax error, but we ignore it and return whatever we have
				// so far
				break;
			}
